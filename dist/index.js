...
const generateMouseEffect = ({ content = "❤", delay = 1900 }) => {
    ...
    const genMouseClickEffect = (e) => {
        if (e.target.tagName === "A") {
            return;
        }
        let x = e.pageX;
        let y = e.pageY;
        const { create, remove, setColor } = generateElement(x, y, content);
        create();
        setColor();
        setTimeout(function () {
            remove();
     ...
     * @param content 元素内容
     */
    const generateElement = (x, y, content) => {
...
exports.default = generateMouseEffect;
